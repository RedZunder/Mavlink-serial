...
#define MODE 2		//0-echo UART  1-heartbeat 1Hz  2-Live decode
...
#if MODE == 0 | MODE == 1
	//sample messages from Wireshark capture (Quadrotor + MissionPlanner)
...
#if MODE==2
	//debug array for saving live bytes from drone
	char bins[300]={0};
...
////////	UART
#if MODE==0	|| MODE==1				//Read from UART and send back
